...
def dosage (tdd, rate=bool, fixed=bool):
    ...
    if fixed:
        ...
        st.caption("Use for patients who are not yet carbohydrate counting, or who have lower cognitive ability. Convert fixed units per meal to fixed grams of carbohydrate per meal and provide dosing instructions for small, medium and large meals.")
        return st.info(f"Fixed insulin per meal (3): **{fixed_dosage} units**")
    else:
...
def carb_ratio (tdd):
    ...
    if st.session_state.weight:
        st.caption("Reduce Pre-Pump by 25% or less if pre-pump TDD >70%")
        st.info(f"Method 1 using 450 RULE: **{m1_carb_ratio} g/unit insulin**")
        st.info(f"Method 2 using weight: **{wt_carb_ratio} g/unit insulin**")
        return m1_carb_ratio
    else:
        
        return st.info(f"Method 1 using 450 RULE: **{m1_carb_ratio} g/unit insulin**")
